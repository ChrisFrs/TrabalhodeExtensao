...
layout_menu = [
    ...
    [sg.Button("Novo Registro"), [sg.Button('Exportar para CSV')], [sg.Button('Importar Excel')]],
    [sg.Text("Registros:")],
...
def janela_update(dados):
    layout_update = [
        ...
        [sg.Text("Observação"), sg.Input(key="observacao")],
        [sg.Button("Atualizar"),sg.Button("Ver Histórico"), [sg.Button('Exportar para CSV')], sg.Button("Excluir"), sg.Button("Cancelar")]
    ]
...
def export_to_csv():
    ...
    conn = sqlite3.connect('registros.db')
    cursor = conn.cursor()
    # Consulta SQL com JOIN para pegar o nomes em vez do id
    cursor.execute('''
    ...
    with open('registros_exportados.csv', mode='w', newline='', encoding='utf-8') as file:
        writer = csv.writer(file)
        # Escrevendo os nomes das colunas
        writer.writerow(['ID', 'QR Code Copo', 'QR Code Placa', 'Hora de Entrada', 'ID do Testador', 'Nome do Debugger', 'Hora do Debug', 'ID da Reparadora', 'Hora do Reparo', 'Componentes', 'Observacao',])
        # Escrevendo os dados
...
def export_to_csvat():
    ...
    with open('registros_modificoes.csv', mode='w', newline='', encoding='utf-8') as file:
        writer = csv.writer(file)
        # Escrevendo os nomes das colunas
        ...
        if file_path:
            try:
                ...
                for index, row in df.iterrows():
                    ...
                    # Inserir no banco de dados com os demais campos vazios
                    cursor.execute('''
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    ''', (qrcode_copo, qrcode_placa, datetime.now().strftime("%Y-%m-%d %H:%M:%S"), None, None, None, None, None, None, None))
                ...
                sg.popup('Importação concluída com sucesso!')
            except Exception as e:
                sg.popup(f'Erro ao importar o Excel: {e}')

    
    if evento == 'Exportar para CSV':
        export_to_csv()
        ...
        if dados:
            ...
            while True:
                ...
                if evento_update == "Atualizar":
                    ...
                    debug_time = registro_atual[6] if registro_atual[6] else (datetime.now().strftime("%Y-%m-%d %H:%M:%S") if valores_update['debug'] else '')

                    # Verifica e atualiza repair_time apenas se estiver vazio
                    repair_time = registro_atual[8] if registro_atual[8] else (datetime.now().strftime("%Y-%m-%d %H:%M:%S") if valores_update['reparadora'] else '')
                ...
                if evento_update == 'Exportar para CSV':
                        export_to_csvat()
